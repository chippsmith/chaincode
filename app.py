...
class ExampleTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        node_1 = self.nodes[1].add_p2p_connection(BaseNode())

        # Node 1 mines a block manually
        self.log.info("Node 1 mines a block")
        ...
        # Node one sends block to peers including node 2
        self.log.info("Node 1 sends block to peers")
        node_1.send_message(block_message)
        ...
        peer_receiving.wait_until(lambda: self.nodes[0].getblockcount() == 12, timeout=5)

        # Check Node 2 recieved the 12th block mined by node 1
        assert(self.nodes[2].getblockcount() == 12)
        self.log.info("Node 2 recieved 12th block from node 1")
